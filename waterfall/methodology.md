...
- **Supported tranche types (v0.x):** senior secured facilities (Term A, Term B, revolver, delayed-draw, accordion), mezzanine, common equity (residual). **Facilities are funded at close only in v0.x** — the opening balance is the funded amount; **mid-life draws are deferred to v0.1** (see Interest/Principal and Limitations). The commitment cap is enforced. **Deferred:** B-piece (pulls in A/B mechanics), preferred equity (quasi-debt promote widens surface without serving the core).
...
- **Facility draws (DEFERRED to v0.1):** mid-life revolver / delayed-draw fundings require a use-of-draw model (drawn cash must fund a designated use and never reach the ECF sweep base or equity). That model is **deferred to v0.1**; **v0.x funds facilities at close only** and **rejects a mid-life `facility_draws` schedule with `UnsupportedFeatureError`** (fail-loud), and enforces the commitment cap. When v0.1 adds it, a draw is a non-CFADS source funding only its designated use — never swept, never distributed.
...
- **Period cash-conservation (sources = uses) — LOCKED (general ledger identity, not a fixed enumeration; resolves pass-6 HIGH):** each period, the sum of **all** cash sources the engine models = the sum of **all** cash uses, within tolerance, or `WaterfallImbalanceError`. **The engine builds both sides from the actual per-period ledger, not from a hardcoded list** — so introducing or exercising any modeled cash mechanism cannot silently falsify the identity (the recurring failure mode of prior passes, where a fixed source list omitted a mechanism modeled elsewhere). Both sides are enumerated from the ledger at assertion time; the following are **illustrative of what the engine currently models, not a closed set**:
  - **Sources (any cash entering the period ledger):** operating CFADS; DSRA/reserve draws; reserve releases; **equity contributions and cash-injection cures** (Cure Rights); applied event proceeds (asset-sale/insurance/etc.); **and, when v0.1 adds it, mid-life facility draws** (deferred in v0.x — facilities are funded at close only). Every non-CFADS source is exactly that — **not** folded into CFADS, which stays operating-only per its definition. (The identity is general and enumerated from the ledger, so v0.1's facility-draw source falls under it automatically.)
...
- **Principal trace:** opening + facility draws (delayed-draw/revolver fundings) − scheduled amort − prepayments − sweeps = closing, per tranche per period. ("Facility draws" here are tranche fundings — distinct from DSRA *reserve* draws; in v0.x mid-life draws are deferred, so this term is 0 and the opening balance carries the at-close funding.) Asserted every period.
...
- **No mid-life facility draws in v0.x — revolver/delayed-draw/accordion are funded at close only (opening balance); mid-life `facility_draws` are rejected (`UnsupportedFeatureError`) and deferred to v0.1 (needs a use-of-draw model so drawn cash never reaches the ECF base or equity); commitment cap enforced**

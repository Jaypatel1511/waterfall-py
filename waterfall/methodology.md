...
**Cash-trap / lock-up behavior (LOCKED — resolves pass-6 MED-1):** a trap-tier covenant breach forces the step-5 ECF sweep to **100%**. Retained ECF is then 0, so **steps 6 and 7 receive no cash** — no junior use, distribution, or residual-to-equity occurs from the operating ladder while the trap is active; the trapped cash prepays senior debt at step 5. This is why lock-up-blocked cash cannot leak to step 7 equity: under a trap there is no retained cash to leak. (The separate proceeds path's equity leg is gated in parallel — see Separate proceeds path below — so neither route reaches equity while debt is outstanding under a trap.) (SPV tax distributions are out of v0.x scope — see Scope — and are not funded from the waterfall; the earlier "permitted at 6c during lock-up" carve-out is removed. Scope and this paragraph now state the same rule.)
